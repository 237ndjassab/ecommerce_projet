...
const WomensBeauty: FC = () => {
  const [mainImage, setMainImage] = useState<string>(PRODUCT_DATA.thumbnail_images[0].src);
  const [selectedColor, setSelectedColor] = useState<string>(PRODUCT_DATA.colors[1].code);
  ...
  // Composant d'affichage des miniatures (inchangé)
  const ThumbnailDisplay: FC<ThumbnailDisplayProps> = ({ src, alt, isSelected }) => (
      <div 
        className={`w-20 h-20 flex items-center justify-center rounded cursor-pointer p-1 
                   ${isSelected ? 'border-2 border-black' : 'border border-gray-300 hover:border-gray-500'}`}
  ...
  return (
    <div className="container mx-auto p-6 md:p-12 font-sans">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Colonne 1 : Images (Utilise le nouveau MainImageDisplay) */}
        ...
        {/* Colonne 2 : Détails du Produit (Inchangée) */}
        <div className="space-y-6">
          <h1 className="text-3xl font-semibold">{PRODUCT_DATA.name}</h1>
          
          {/* Statistiques (Petits icônes) */}
          <div className="text-sm text-gray-500 space-y-1">
            <p className="flex items-center">
                <span className="text-black text-lg mr-1"><FaEye/></span> 
          ...
          <div className="text-sm space-y-1">
            ...
            <p><span className="font-semibold">Product Code:</span> {PRODUCT_DATA.code}</p>
            <p><span className="font-semibold">Category:</span> {PRODUCT_DATA.category}</p>
            <p><span className="font-semibold">Tag:</span> <span className="text-gray-600">{PRODUCT_DATA.tags.join(', ')}</span></p>
          </div>
          
          <p className="text-gray-600 text-sm">{PRODUCT_DATA.description}</p>
          
          <div className="text-4xl font-bold text-gray-900">${PRODUCT_DATA.price}</div>
          
          <hr className="border-gray-200" />
          
          {/* Choix de Couleur */}
          <div className="space-y-2">
            <p className="font-medium text-sm">Color: <span className="font-normal text-gray-600">{PRODUCT_DATA.colors.find((c: Color) => c.code === selectedColor)?.name || 'Unknown'}</span></p>
            <div className="flex space-x-3">
              {PRODUCT_DATA.colors.map((color: Color) => (
                <button
                  key={color.code}
                  onClick={() => setSelectedColor(color.code)}
                  className={`w-6 h-6 rounded-full border-2 ${selectedColor === color.code ? 'border-gray-900 ring-2 ring-offset-2 ring-gray-900' : 'border-gray-300'}`}
          ...
          {/* Choix de Taille */}
          <div className="space-y-2">
            <p className="font-medium text-sm">Size: <span className="font-normal text-gray-600">{selectedSize}</span></p>
            <div className="flex space-x-3">
              {PRODUCT_DATA.sizes.map((size: string) => (
                <button
                  key={size}
                  onClick={() => setSelectedSize(size)}
                  className={`w-9 h-9 border text-sm flex items-center justify-center ${selectedSize === size ? 'bg-black text-white border-black' : 'bg-white text-gray-700 border-gray-300'}`}
