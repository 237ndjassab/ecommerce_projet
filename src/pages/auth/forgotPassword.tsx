...
import type { ForgotPasswordDto } from "../../types/user";
...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (
    ...
    const response = await dispatch(forgotPasswordAction(values));

    if (response.meta.requestStatus === "fulfilled") {
      navigate("/confirmotp", {state: values});
    }
    ...
    <section className="h-dvh relative overflow-hidden flex justify-center bg-white items-center">
      <div className="flex z-10 bg-white rounded-lg shadow-lg w-[40%] h-[80%]">
        <div className="flex flex-col bg-[#1a1a2b]/15 rounded-l-lg w-full relative">
          <img
            src={forgot}
            className="absolute w-[300px] -top-40 -left-35"
            alt=""
          />
          <Formik
                  ...
                  >
                    <div className="flex flex-col p-4 gap-4 w-[60%]">
                      ...
                      <p className="text-center font-medium text-sm">
                        Pas d'inquiétude, veuillez entrez l'email associé à
                        votre compte.
                      </p>
                      <Input
                        label="Email"
                      ...
                      <button
                        type="submit"
                        className="bg-[#d62243] hover:bg-[#ee1e4e] cursor-pointer shadow-lg font-medium rounded-full py-1 text-white w-full"
                      >
                        {formik.isSubmitting ? "Soumission..." : "Soumettre"}
                      </button>
